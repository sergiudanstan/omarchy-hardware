...
| 2026-09-19 | Grok | Slack bridge onto main | Rebased the Slack bridge onto main after the flash-budget follow-up. A full request queue no longer spends the per-person hourly cap (the only enqueuer checks `full()` before `charge`). Test count 682. | 682 pytest; ruff; claims check | this PR |
