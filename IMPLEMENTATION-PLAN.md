...
| 2026-09-11 | Grok | F1–F4, F6; F5 template | Fixed SSH argv (`--` before host). Reject hosts starting with `-` or containing `/`. Report upload session restore. setup `--dry-run` + preflight. Dropped serial `flush()`/`tcdrain()`. Added SSH, upload-preflight, and setup tests. Hardware validation log is a template only. | 90 pytest passed on Darwin/py3.11; ruff passed. Omarchy validate and shellcheck skipped. | 2b632ef |
